  ...
  return (
    <div className="relative">
      <button
        ...
        className="relative rounded-md p-2 text-[color:var(--color-text-body)] hover:bg-[color:var(--color-bg-subtle)]"
      >
        <Bell className="h-5 w-5" aria-hidden="true" />
        {unreadCount > 0 && (
          <span
            className="absolute right-1 top-1 h-2 w-2 rounded-full bg-[color:var(--color-feedback-error)]"
            aria-hidden="true"
          />
        )}
      </button>
          ...
          <div className="flex items-center justify-between border-b border-[color:var(--color-border-default)] p-3">
            ...
            {unreadCount > 0 && (
              <button
                ...
                className="text-overline text-[color:var(--color-brand-primary)]"
              >
                Mark all read
              </button>
            )}
          ...
          <div className="max-h-80 overflow-y-auto">
            ...
            {items.map((n) => (
              <button
                ...
                className={`flex w-full flex-col gap-0.5 border-b border-[color:var(--color-border-default)] p-3 text-left last:border-0 ${n.readAt ? '' : 'bg-[color:var(--color-brand-tint)]'}`}
              >
                <p className="text-small font-medium text-[color:var(--color-text-primary)]">
                  {n.title}
                </p>
                {n.body && (
                  <p className="text-small text-[color:var(--color-text-muted)]">{n.body}</p>
                )}
                <p className="text-overline text-[color:var(--color-text-muted)]">
