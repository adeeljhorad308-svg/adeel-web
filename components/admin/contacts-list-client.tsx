...
import { updateContactRequest, deleteContactRequest, exportContactRequestsCsv } from '@/lib/actions/contact-actions';
...
export function ContactsListClient({ initial }: { initial: ContactRequest[] }): React.ReactElement {
  ...
  if (rows.length === 0) {
    return <EmptyState title="No contact requests yet" description="Submissions from your public contact form will appear here." />;
  }
    ...
    <>
      <div className="mb-4 flex items-center justify-between">
        <FilterToolbar
          ...
          selects={[{ param: 'replyStatus', label: 'Status', options: [{ value: 'NEW', label: 'New' }, { value: 'REPLIED', label: 'Replied' }, { value: 'CLOSED', label: 'Closed' }] }]}
        />
        <Button variant="secondary" onClick={() => void exportCsv()}><Download className="h-4 w-4" aria-hidden="true" />Export CSV</Button>
      </div>
      <DataTable<ContactRequest>
        ...
        columns={[
          ...
          { key: 'service', header: 'Service', render: (r) => r.serviceInterest ?? '—', hideOnMobile: true },
          { key: 'status', header: 'Status', render: (r) => <StatusPill status={r.replyStatus === 'NEW' ? 'DRAFT' : r.replyStatus === 'REPLIED' ? 'PUBLISHED' : 'ARCHIVED'} /> },
          { key: 'date', header: 'Received', render: (r) => <ClientFormattedDate date={r.createdAt} format="date" />, hideOnMobile: true },
        ]}
        rowActions={(row) => (
          <div className="flex justify-end gap-1">
            ...
            <button onClick={() => convert(row)} aria-label={`Convert ${row.name} to lead`} className="rounded-md p-2 text-[color:var(--color-brand-primary)] hover:bg-[color:var(--color-bg-subtle)]">
              <ArrowRightCircle className="h-4 w-4" aria-hidden="true" />
            </button>
            <button onClick={() => setPendingDelete(row)} aria-label={`Delete request from ${row.name}`} className="rounded-md p-2 text-[color:var(--color-feedback-error)] hover:bg-[color:var(--color-bg-subtle)]">
              <Trash2 className="h-4 w-4" aria-hidden="true" />
            </button>
          </div>
        )}
      />
      <ConfirmDialog open={pendingDelete !== null} title="Delete this request?" description="This cannot be undone." onConfirm={handleDelete} onCancel={() => setPendingDelete(null)} loading={isPending} />
    </>
  );
}
